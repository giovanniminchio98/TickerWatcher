...
def _build_prompt(snapshot):
    ...
    return (
        ...
        "comparison post built around stock numbers at all. Confirmed live that pairing live-moving "
        "crypto against a frozen Friday close (e.g. 'crypto decouples from stocks' as if both are "
        "moving right now) reads as a misleading live cross-asset story even when the tense is "
        "technically correct, because one side of the comparison isn't actually moving. Only bring "
        "weekend stock numbers into a post when it's genuinely anchored to a specific news story -- "
        "a real news_index, an actual earnings/press-release item, something worth explaining on its "
        "own merits -- never as a routine side-by-side snapshot. Crypto-only posts are always fine "
        "on weekends, with no such restriction.\n\n"
