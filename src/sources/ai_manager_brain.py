...
def _build_prompt(snapshot):
    ...
    return (
        ...
        "'today' is always accurate for a crypto price move, weekend or not. Applies to second_part "
        "exactly as much as the main post -- confirmed live that an otherwise-good, genuinely "
        "news-anchored main post (a real story, correctly free of any weekend stock issue) had this "
        "rule broken by its OWN second_part instead, tacking on an unrelated 'meanwhile crypto vs "
        "stocks' aside as a bonus tidbit. The main post being exempt (because it's news-anchored) "
        "never exempts a second_part that introduces a fresh, unrelated weekend stock comparison of "
        "its own -- second_part is subject to every rule below independently, not inherited slack "
        "from whatever the main post happened to qualify for.\n"
        "- On a weekend, this goes beyond tense: avoid a routine 'crypto vs stocks' market-pulse "
        "comparison post OR second_part built around stock numbers at all. Confirmed live that "
        "pairing live-moving crypto against a frozen Friday close (e.g. 'two different worlds moving "
        "on two different clocks right now') reads as a misleading live cross-asset story even when "
        "the tense is technically correct, because one side of the comparison isn't actually moving. "
        "Only bring weekend stock numbers into a post or second_part when IT ITSELF is genuinely "
        "anchored to a specific news story -- a real news_index, an actual earnings/press-release "
        "item, something worth explaining on its own merits -- never as a routine side-by-side "
        "snapshot or a generic 'meanwhile' aside bolted onto an unrelated main story. Crypto-only "
        "posts and second_parts are always fine on weekends, with no such restriction.\n\n"
