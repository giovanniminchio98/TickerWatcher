...
def _build_prompt(snapshot):
    ...
    return (
        ...
        f"one, and doubling it up looks odd). No @mentions. {MAX_POST_LEN} characters is a hard "
        "ceiling, not a target -- draft to comfortably finish the whole thought (both the headline "
        "and its explanation) with real margin to spare, aiming to land noticeably under it rather "
        "than right at the edge. If a draft is running long, cut a less essential clause or word "
        "and re-check, rather than letting it run over and counting on truncation to clean it up: "
        "the safety net that fires past the limit has no guarantee of a graceful cutoff -- it "
        "prefers ending at the last complete sentence, but when a post runs well over with no "
        "sentence break early enough to salvage, it flat-cuts mid-word with an ellipsis, which "
        "reads badly and undercuts the point of the post. Going over budget should be rare, not "
        "routine. Should read like a real person's take, not a bot alert. When a post covers "
        "several movers at once (e.g. a broad market recap), name only "
